...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Nombres de los reportes que usamos como callback_data
    lista_reportes = [
        "Contratos a Vencer",
        "Contratos a Incrementar Portafolios Externos",
        "Contratos a Incrementar "
    ]
    ...
    # Lógica para mostrar los reportes
        ...
        await query.edit_message_text(text="Elige un reporte para cargar en Unifier:", reply_markup=reply_markup)

    # Si el botón pulsado es uno de los reportes de la lista
    elif query.data in lista_reportes:
        nomReporte = query.data  # ¡Aquí obtenemos el nombre directamente del botón!
        await query.edit_message_text(text=f"Cargando reporte: '{nomReporte}'. Espera por favor...")
        
        try:
            # Llamamos a la función con el nombre del reporte que viene en el botón
            ...
            # Enviamos un nuevo mensaje porque el anterior ya lo editamos
            await query.message.reply_text("Reporte cargado. Ahora puedes hacerme una pregunta.")
        except Exception as e:
            await query.edit_message_text(f"Error cargando el reporte: {str(e)}")
